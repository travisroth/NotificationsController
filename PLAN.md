...
8. Storage settings (saving history, retention, limits, duplicate window) are global, in settings.json, not in NVDA's configuration profiles, so a profile switch cannot start writing notifications to disk or delete history. They fail closed: a damaged or unreadable file means history stays in memory only, and a failed save is reported to the user. Changing where history is saved is all or nothing: the existing file is read and merged before anything changes, so a file that cannot be read is never appended to or overwritten, and history stays in memory only. Problems are tracked separately (settings unreadable, settings not saved, history unreadable, history not deleted) so each warning describes what is actually happening.
